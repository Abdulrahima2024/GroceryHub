        ...
        {myOrders.map((order, index) => (
            <div key={index} className='border border-gray-300 rounded-lg mb-10 p-4 py-5 max-w-4xl'>
                ...
                {order.items.map((item, index) => {
                    ...
                    const imgSrc = prod.image?.[0] || prod.images?.[0] || '';
                    const unitPrice = Number(prod.offerprice ?? prod.offerPrice ?? prod.price ?? 0);
                        ...
                        <div className='flex items-center mb-4 md:mb-0'>
                            <div className='bg-primary/10 p-4 rounded-lg'>
                                {imgSrc ? (
                                  ...
                                  <div className='w-16 h-16 bg-gray-100 flex items-center justify-center text-sm text-gray-400'>No Image</div>
                                )}
                            </div>
                            <div className='ml-4'>
                                <h2 className='text-xl font-medium text-gray-800'>{prod.name || 'Product'}</h2>
